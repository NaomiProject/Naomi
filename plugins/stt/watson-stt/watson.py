...
class IBMWatsonSTTPlugin(plugin.STTPlugin):
    """
    Speech-To-Text implementation which relies on
    the IBM Watson Speech To Text API.
    This implementation requires an IBM Cloud Speech-To-Text API key to be
    present in profile.yml
    ...
    def transcribe(self, fp):
        """
        Performs STT via the IBM Watson Speech-To-Text API,
        ...
        if not self.username:
            self._logger.critical('API username missing, transcription request'
                                  + ' aborted.')
            return []
        elif not self.password:
            self._logger.critical('API password missing, transcription request'
                                  + ' aborted.')
            return []
        ...
        try:
            ...
            self._logger.critical('Request failed with http status {status_c}'
                                  ).format(
