...
class IBMWatsonTTSPlugin(plugin.TTSPlugin):
    """
        Text-To-Speech implementation which relies on the IBM Watson
        Text To Speech API.

        This implementation requires an IBM Cloud Text-To-Speech API key
        to be present in profile.yml
        ...
        6. Set the value of the 'tts_engine' key in
           your profile.yml to 'watson'
    ...
    def say(self, phrase):
        """
            Performs TTS via the IBM Watson Text-To-Speech API,
            synthesizing voice audio and returning an audio file.
        ...
        if not self.username:
            self._logger.critical('API username missing, '
                                  + 'synthesize request aborted.')
            return []
        elif not self.password:
            self._logger.critical('API password missing, '
                                  + 'synthesize request aborted.')
            return []
        ...
        try:
            ...
            self._logger.critical('Request failed with http status'
                                  + '{status}: {status_text}'
                                  ).format(
